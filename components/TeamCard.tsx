"use client";
import { Card, CardFooter, CardHeader, Divider} from "@nextui-org/react";
import Image from "next/image";

const TeamCard = () => {
  return (
    <Card shadow="sm" className="w-[260px] md:w-[320px] first:ms-5 bg-primary text-white">
      <CardHeader className="flex items-start gap-5">
        {/* <Image
          alt="nextui logo"
          height={120}
          width={60}
          radius="lg"
          src="/therapy2.jpg"
        /> */}
        <div className="relative w-[105px] h-[105px]">
          <Image fill alt="Team member" src={"/therapy2.jpg"}  className="object-cover object-center rounded-md"  sizes="(min-width: 780px) calc(49.1vw - 43px), calc(100vw - 48px)"
          placeholder="blur"
          blurDataURL="data:image/jpeg;base64,"/>
        </div>
        <div className="flex flex-col font-Lato font-medium">
          <p className="font-Montserrat font-semibold text-lg md:text-2xl">Name</p>
          <p className="text-base font-Lato font-normal">Designation</p>
          <p className="text-base font-Lato font-normal">Role</p>
          <p className="text-base font-Lato font-normal">@speciality</p>
        </div>
      </CardHeader>
      <Divider className="bg-white"/>
      {/* <CardBody className="overflow-visible p-0 relative w-full h-[240px]">
        <Image
        fill
          alt={"team members"}
          className="object-cover object-center"
          src={"/therapy2.jpg"}
        />
      </CardBody> */}
      <CardFooter className="text-base">
        <p className="font-Lato">Lorem ipsum dolor sit, amet consectetur adipisicing elit.</p>
      </CardFooter>
    </Card>
  );
};

export default TeamCard;
